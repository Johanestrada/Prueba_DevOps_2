...
export const FormCierreDespacho = ({ despacho, onClose }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await api.put(`/despachos/${despacho.idDespacho}`, jsonData);
      Swal.fire({
  ...
  return (
    ...
    >
      ...
      <div className="mb-5">
        <label className="block font-bold mb-2">Patente Camion</label>
        <input
          type="text"
          disabled={true}
      ...
      <div className="mb-5">
        <label className="block font-bold mb-2">Direccion Compra</label>
        <input
          type="text"
          className="border border-gray-300 rounded-lg block w-full text-slate-400 p-1"
          disabled={true}
