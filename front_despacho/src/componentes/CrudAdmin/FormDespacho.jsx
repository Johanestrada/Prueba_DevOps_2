...
export const FormDespacho = ({ venta, onClose }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await api.put(`/ventas/${venta.idVenta}`, jsonDataSales);
      await api.post("/despachos", jsonData);
      Swal.fire({
        title: "Despacho registrado",
        text: "El despacho ha sido generado con exito en la base de datos",
        icon: "success",
  ...
  return (
    ...
    >
      ...
      <div className="mb-5">
        <label className="block font-bold mb-2">Patente de camion</label>
        <input
          type="text"
          placeholder="Elige patente de camion"
          className="border border-gray-300 rounded-lg block w-full p-1"
